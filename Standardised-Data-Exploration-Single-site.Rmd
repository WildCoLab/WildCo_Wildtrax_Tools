title: "Data Exploration" date: "`r format(Sys.time(), '%d %B, %Y')`" output: html_document editor_options: chunk_output_type: console ---
...
dat <- read.csv("../1. Master data/Tuyeta_CAM_Detection_Data_v5.csv", header=T)
str(dat)
dat$species_rank<-as.factor(dat$species_rank)
dat$common_name<-as.factor(dat$common_name)
summary(dat$species_rank)
summary(dat$common_name)
# drop detections that aren't ID-ed to at least the species level:
dat<-droplevels(dat[dat$species_rank %in% c("Species","Subspecies"),])


eff <- read.csv("../1. Master data/Tuyeta_CAM_Deployment_Data_v5.csv", header=T)
...
eff$mistake<-(strptime(eff$Camera.Deployment.End.Date, "%Y-%m-%d", tz="UTC")-strptime(eff$Camera.Deployment.Begin.Date, "%Y-%m-%d", tz="UTC"))>=0
...
det.sum.total <- as.data.frame(count(dat[dat$Blank==FALSE & dat$Species!="NONE",], Species))
...
for(j in 1:length(unique(eff$Plot.grp)))
{
    ...
    for(i in 1:length(unique(eff$Deployment.Location.ID[eff$Plot.grp==j])))
    {
      ...
      tmp2 <- dat[dat$location==tmp$Deployment.Location.ID[1],]
...
dat <- dat[dat$Blank==FALSE & dat$Species!="NONE",]
dat$Species <- as.character(dat$Species)
dat$location <- as.character(dat$location)

# Order the datframe by Site, date
dat <- dat[order(dat$location, dat$location),]


### NEW WAY
  dat <- dat %>%
  #filter(Species == i) %>%
  arrange(Project.ID,location, Date_Time.Captured) %>%
  group_by(location, Species) %>%
  mutate(duration = int_length(lag(Date_Time.Captured) %--% Date_Time.Captured) )
...
par(mfrow=c(1,1))
tmp <- as.data.frame.matrix(table(ind.dat$location, ind.dat$Species))
...
det.sum.site <- as.data.frame(table(ind.dat$location, ind.dat$Species))
colnames(det.sum.site) <- c("location","Species", "Detections")
det.sum.site$Individuals <- NA

i <- 1
for(i in 1:nrow(det.sum.site))
{
   tmp <- subset(ind.dat, location==as.character(det.sum.site$location)[i] &
...
CR.site <- left_join(det.sum.site,aggregate(Days~Deployment.Location.ID, data=eff,  FUN=sum, na.rm=T), by = c("location" = "Deployment.Location.ID") )
CR.site$CR.100 <- round((CR.site$Individuals/CR.site$Days)*100,3)
# Add station locations
CR.site <- left_join(CR.site, sta[, c("location", "Latitude", "Longitude")])
CR.site<-CR.site[CR.site$Days>0,]
...
```{r setup the map, include=F}

tuyeta <- st_read("H:/Research_Labs/Burton_Lab/Camera_Trap_Projects/Active Projects/Northwest Territories/3. Data/3.2 GIS/Tuyeta_boundary/Tuyeta.shp")
tuyeta <- st_transform(tuyeta, crs = 3580)

data=st_read("C:/Users/laura/Documents/Wildco/3. Data/SingleSiteExploration/Tuyeta Biodiversity Project_30min_Independent.csv",
        options=c("X_POSSIBLE_NAMES=Longitude","Y_POSSIBLE_NAMES=Latitude"))


caribou<-data[data$Species.Common.Name=="Woodland Caribou",]
caribou$Species.Common.Name<-rep("Caribou",nrow(caribou))
data<-data[data$Species.Common.Name!="Woodland Caribou",]
data<-rbind(data,caribou)

data$Species.Common.Name<-as.factor(data$Species.Common.Name)
data$location<-as.factor(data$location)
spatdata<-data %>% 
  group_by(Species.Common.Name, location) %>%
...
species<-unique(data$Species.Common.Name)
i<-1

for (i in 1:length(species)){
  tmp<-spatdata[spatdata$Species.Common.Name==species[i],]
  ...
  x=tm_shape(tuyeta) +
    ...
    tm_shape(tmp) +
    tm_bubbles("detections",
               col ='blue',
               breaks = c(1, 5, 10, max(tmp$detections)),
               size.max = 10,
               sizes.legend= c(1, 5, 10, if(max(tmp$detections)>10) max(tmp$detections))) +
