...
dat <- read.csv("../1. Master data/Tuyeta_CAM_Detection_Data_v6.csv", header=T)
str(dat)
...
for(i in 1:nrow(tmp))
{
  ...
  {
    daily.lookup[[i]] <- data.frame("Date"=seq(as.Date(tmp$Camera.Deployment.Begin.Date[i])+1, as.Date(tmp$Camera.Deployment.End.Date[i]), by="days"), "Deployment.Location.ID"=tmp$location[i])
  }
}
